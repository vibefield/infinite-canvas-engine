/**
 * The gl-board document HUD: undo/redo + autosave status panel, plus the
 ...
 * Escape cancels any active gesture (design-003 §8). Adapted from card-board's.
 */
...
export function createDocUi(deps: DocUiDeps): DocUi {
  ...
  const reflector: ReflectorDef = {
    ...
    flush() {
      ...
      const lines = [
        `gl-board · ${deps.widgetCount} widgets${deps.session.readOnly ? " [READ-ONLY]" : ""}`,
        "tap a cube to toggle spin · drag anywhere else",
        `autosave: ${savedLabel()}`,
      ];
