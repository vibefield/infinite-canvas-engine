 ...
 * bad save never blocks the next boot (the M5 never-brick path). Copied verbatim
 * from the gl-board demo — the storage contract is app-side and identical here.
 */
import type { AutosaveStorageRead, AutosaveStorageWrite } from "@ice/core";

export interface NodeboardStorage extends AutosaveStorageRead, AutosaveStorageWrite {
...
export function createLocalStorageStorage(key: string): NodeboardStorage {
  return {
