 ...
 */
import { NO_MODS, Viewport, defineQuery, type Entity, type InputEvent, type World } from "@ice/core";
...
describe("widgetlab cursor halo", () => {
  it("morphs ring → MID over a card → DOT over an internal interactive → back", () => {
    ...
    ce.stack.queue.enqueue(ev({ kind: "move", screenX: 600, screenY: 900, overInteractive: false }));
    step(60);
    ...
    ce.stack.queue.enqueue(ev({ kind: "move", screenX: 127, screenY: 127, overInteractive: false }));
    step(60);
  ...
  it("touch pointers get no halo (nothing to augment)", () => {
    ...
    ce.stack.queue.enqueue(
      ev({ kind: "down", pointerId: "touch:1", device: "touch", screenX: 300, screenY: 300, buttons: 1 }),
    );
    step(2);
