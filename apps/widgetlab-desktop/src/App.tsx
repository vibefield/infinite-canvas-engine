...
/** Development/E2E console probe: `window.__iceDebug()` dumps the live snap state. */
...
export function App() {
  ...
  // committed ce only.
  useEffect(() => {
    const requestedByDesktopSmoke = new URLSearchParams(window.location.search).get("rendererDebug") === "1";
    if (import.meta.env.DEV || requestedByDesktopSmoke) installDebugProbe(ce);
  }, [ce]);
