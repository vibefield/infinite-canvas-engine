 ...
 *
 * DOC-LESS by design: widgetlab has no `vitest.config.ts` yet (task 65), so
 * it lacks the `loro-crdt → loro-crdt/base64` alias every sibling app's vitest
 * config carries; `docs.create()` would build a `LoroDoc` from the wrong loro
 * build and throw. Doc-less construction never touches loro at runtime, and the
 * panels handle the doc-less case (undo-status resource absent ⇒ flags false).
 * The environment comes from the docblock pragma above; the JSX transform for
