 ...
 *    equality.
 */
import { NavDepth, NavFrame, PrefabId, type CanvasEngine, type Entity, defineQuery, widgets } from "@ice/core";
...
function readCrumbs(engine: CanvasEngine): Crumb[] {
  ...
  engine.world.query(navEntryQ).each((b) => {
    for (const r of b) {
      ...
      entries.push({
        depth,
        container: container !== undefined && engine.world.isAlive(container) ? container : undefined,
      });
    }
  });
    ...
    <div className="absolute top-4 left-4 z-50 flex items-center gap-2">
      ...
      >
        {crumbs.map((crumb, i) => {
          ...
          return (
            <div key={`${crumb.depth}-${crumb.containerId ?? "root"}`} className="flex items-center">
              {i > 0 && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="12"
