...
import { useWidgetProps, type WidgetComponentProps } from "@ice/react";
...
const WEATHER_CONDITIONS: readonly WeatherCondition[] = ["sunny", "cloudy", "partly-cloudy", "rainy"];
  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-col justify-between p-4">
        <div>
          <div className="text-[11px] font-medium uppercase tracking-wider opacity-80">{location}</div>
          <div className="text-[10px] opacity-60">Partly Cloudy</div>
        </div>
        <div className="flex items-end gap-2">
          <span className="text-4xl font-light tabular-nums leading-none">{Math.round(temp)}</span>
...
export const WeatherCard = defineWidget({
  ...
  provides: ["widget"],
  interaction: { solid: true, dragOn: "longPress", resizable: false },
});
