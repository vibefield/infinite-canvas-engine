...
function OrbitCubeView({ entity, world }: WidgetComponentProps): ReactElement {
  const props = useWidgetProps<OrbitCubeProps>(world, entity, "orbit-cube-card");
  ...
  const onPointerDown = useCallback((e: GLPointer) => {
    ...
    e.stopPropagation();
    moved.current = 0;
    drag.current = { pointerId: e.nativeEvent.pointerId, lastX: e.nativeEvent.clientX, lastY: e.nativeEvent.clientY };
  }, []);
