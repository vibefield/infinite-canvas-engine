...
- pnpm monorepo (Node 24+): packages `kernel`/`core`/`dom`/`react`/`r3f`/`ground`/`devtools` (`@ice/*` — scope rename possible pre-publish). Import walls are dependency-cruiser-enforced and CI-fatal: kernel imports nothing (single named exception: `rbush`); core = strata-ecs + kernel only; chain dom → react → r3f; devtools → core; ground = core + kernel + three (the P0 ground layer: ONE WebGPU canvas — three WebGPURenderer + TSL, WebGL2 fallback — grid/wires/snap-guides/selection-ring passes); nobody imports devtools or ground (react gets the ground layer as an opaque `<InfiniteCanvas ground={ground(...)}>` factory prop; imperative apps register its reflector).
- Commands: `pnpm run ci` (typecheck + lint + test + depcruise — the merge gate) · `pnpm run build` · per-package `pnpm --filter @ice/kernel test`.
- strata-ecs pinned at `0.7.0` (pre-1.0; bumped 2026-07-14 for `world.changes` ChangeCollectors — petition 7, additive release, full suite re-run green): API-verify against `../strata-ecs/src` when in doubt (the design docs cite file:line for every load-bearing claim); upgrades re-run the full trace suite. 0.3.0–0.7.0 shipped ALL SIX engine petitions (`{undoable:false}` tx; observer precision; `orderIndependent` attestation; `metaTransaction`; `devOnWrite`; `defineTickSystem` + attributed `ctx.query` + zero-match chunk skip; pull-based `ChangeCollector` — spatialSync is O(delta) now) — record new ones in `docs/strata-petitions.md` (one file per petition in `docs/petitions/`).
