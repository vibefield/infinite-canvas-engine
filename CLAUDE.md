...
- strata-ecs pinned at `0.6.0` (pre-1.0; bumped 2026-07-13 for `/tools` `attachProfiler` — additive release, full suite re-run green): API-verify against `../strata-ecs/src` when in doubt (the design docs cite file:line for every load-bearing claim); upgrades re-run the full trace suite. 0.3.0–0.5.0 shipped ALL FIVE engine petitions (`{undoable:false}` tx; observer precision; `orderIndependent` attestation; `metaTransaction`; `devOnWrite`; `defineTickSystem` + attributed `ctx.query` + zero-match chunk skip) — record new ones in `docs/strata-petitions.md` (one file per petition in `docs/petitions/`).
